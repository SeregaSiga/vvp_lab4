...
//задание номер 2
 int A,B,C;
 cout<<'введите любые три точки (A,B,C) ,чтобы узнать расстояние отрезков AC и BC '<<endl;
 cout<<'A'<<' '<<'B'<<' '<<'C'<<endl;
 cin>>A>>B>>C;
 cout<<'A'<<'C'<<'='<<(abs(A)+abs(C))<<' '<<'B'<<'C'<<'='<<(abs(B)+abs(C))<<endl;
 //задание номер 3
 int a,b,c;
 int b1;
 ...
 cin>>a>>b>>c;
 if (a<c<b&&b<c<a)
  cout<<'P'<<'='<<(abs(a)+abs(c))*(abs(b)+abs(c)<<endl;
  else
  {
    b1=b;
    b=c;
